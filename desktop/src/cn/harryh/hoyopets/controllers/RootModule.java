 ...
 * At GPL-3.0 License
 */
package cn.harryh.hoyopets.controllers;

import cn.harryh.hoyopets.HoyoConfig;
import cn.harryh.hoyopets.HoyoHomeFX;
import cn.harryh.hoyopets.EmbeddedLauncher;
import cn.harryh.arkpets.concurrent.ProcessPool;
import cn.harryh.hoyopets.guitasks.DeleteTempFilesTask;
import cn.harryh.hoyopets.guitasks.GuiTask;
...
public final class RootModule implements Controller<HoyoHomeFX> {
    ...
    private HoyoHomeFX app;
    private double xOffset;
    private double yOffset;

    @Override
    public void initializeWith(HoyoHomeFX app) {
        this.app = app;
        ...
        // Load config file.
        app.config = Objects.requireNonNull(HoyoConfig.getConfig(), "ArkConfig returns a null instance, please check the config file.");
    ...
    /** Runs the EmbeddedLauncher to launch the HoyoPets app.
     * It will run in multi-threading mode.
     * @see EmbeddedLauncher
     */
    public void startHoyoPetsCore() {
        Task<Boolean> task = new Task<>() {
            ...
            protected Boolean call() throws InterruptedException, ExecutionException {
                ...
                // HoyoPets core finalized.
                if (!future.get().isSuccess()) {
                    int exitCode = future.get().exitValue();
                    Logger.warn("Launcher", "Detected an abnormal finalization of an HoyoPets thread (exit code " + exitCode + "). Please check the log file for details.");
                    lastLaunchFailed = new ProcessPool.UnexpectedExitCodeException(exitCode, future.get().processId());
                    return false;
                }
                Logger.debug("Launcher", "Detected a successful finalization of an HoyoPets thread.");
                return true;
            }
        };
        Thread thread = new Thread(task);
        task.setOnFailed(e ->
                Logger.error("Launcher", "Detected an unexpected failure of an HoyoPets thread, details see below.", task.getException())
        );
        thread.start();
    }

    /** Fetches a regular check-app-up-date request from the HoyoPets server.
     */
...
//        new CheckAppUpdateTask(app.body, GuiTask.GuiTaskStyle.HIDDEN, "auto").start();
    ...
    private void initAnnoEntrance() {
        ...
        try {
            FXMLHelper.LoadFXMLResult<HoyoHomeFX> fxml = FXMLHelper.loadFXML("/_UI/AnnounceDialog.fxml");
...
//        });
    ...
    private static class TrayExitHandBook extends Handbook {
        ...
        public String getContent() {
            return "看来你已经启动了你的第一个 HoyoPets 桌宠！尽情享受 HoyoPets 吧！";
        }
