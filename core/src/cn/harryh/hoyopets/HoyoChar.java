...
import cn.harryh.hoyopets.animations.AnimClip;
import cn.harryh.hoyopets.animations.AnimClip.AnimStage;
import cn.harryh.hoyopets.animations.AnimClipGroup;
import cn.harryh.hoyopets.animations.AnimComposer;
import cn.harryh.hoyopets.animations.AnimData;
import cn.harryh.hoyopets.assets.ModelItem.ModelAssetAccessor;
...
public class HoyoChar {
    ...
    /** Initializes an HoyoPets character.
     * @param config The HoyoPets Config instance which contains the asset's information and other essential settings.
     * @param scale The scale of the skeleton.
     */
    public HoyoChar(HoyoConfig config, float scale) {
        // 1.Graphics setup
        ...
        // 2.Geometry setup
        EasingFunction easingFunction = HoyoConfig.getEasingFunctionFrom(config.transition_type);
        ...
        try {
            ...
            ModelAssetAccessor modelAssetAccessor = new ModelAssetAccessor(config.character_asset);
            ...
            // Load atlas with correct encoding
            FileHandle packFile = Gdx.files.internal(path2atlas);
            FileHandle tempFile = FileHandle.tempFile("atlas-converted-");
            tempFile.writeString(packFile.readString("UTF-8"), false, Charset.defaultCharset().name());
            TextureAtlas.TextureAtlasData atlasData = new TextureAtlas.TextureAtlasData(tempFile, packFile.parent(), false);
            ...
            throw new RuntimeException("Launch HoyoPets failed, the model asset may be inaccessible.");
        }
        skeleton = new Skeleton(skeletonData);
        skeleton.updateWorldTransform(Skeleton.Physics.update);
        Array<Skin> skins = skeletonData.getSkins();
        skeleton.setSkin(skins.get(skins.size - 1));
        ...
        // 6.Canvas setup
        setCanvas(HoyoConfig.getGdxColorFrom(config.canvas_color));
        outlineWidth = config.render_outline_width;
        outlineColor = HoyoConfig.getGdxColorFrom(config.render_outline_color);
        shadowColor = HoyoConfig.getGdxColorFrom(config.render_shadow_color);
    ...
    private ShaderProgram getShader(String path2vertex, String path2fragment, boolean gles30) {
        ...
        if (!shader.isCompiled()) {
            ...
            throw new RuntimeException("Launch HoyoPets failed, failed to compile shaders.");
        }
        return shader;
    }
