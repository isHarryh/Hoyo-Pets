...
import cn.harryh.hoyopets.HoyoChar;
import cn.harryh.hoyopets.HoyoConfig;
import cn.harryh.hoyopets.animations.AnimData;
import cn.harryh.hoyopets.animations.GeneralBehavior;
import cn.harryh.hoyopets.concurrent.SocketClient;
...
import cn.harryh.hoyopets.tray.MemberTrayImpl;
...
import static cn.harryh.hoyopets.Const.coreTitleManager;


public class HoyoPets extends ApplicationAdapter implements InputProcessor {
    /* RENDER PROCESS */
    public Plane plane;
    public HoyoChar cha;
    ...
    public void create() {
        ...
        config = Objects.requireNonNull(HoyoConfig.getConfig(), "ArkConfig returns a null instance, please check the config file.");
        ...
        cha = new HoyoChar(config, config.display_scale);
        behavior = new GeneralBehavior(config, cha.animList);
        offsetY = (int) (behavior.defaultAnim().animClip().type.offsetY * config.display_scale);
        ...
        HoyoConfig.Monitor primaryMonitor = refreshMonitorInfo();
        ...
        windowPosition = new TransitionVector2(
                HoyoConfig.getEasingFunctionFrom(config.transition_type),
    ...
    public void render() {
        ...
        // 4.Outline.
        HoyoConfig.RenderOutline renderOutline = HoyoConfig.getRenderOutlineFrom(config.render_outline);
        cha.setOutlineAlpha(renderOutline == HoyoConfig.RenderOutline.ALWAYS ||
                mouseStatus.mouseDown && renderOutline == HoyoConfig.RenderOutline.PRESSING ||
                isFocused && renderOutline == HoyoConfig.RenderOutline.FOCUSED ||
                mouseStatus.dragging && renderOutline == HoyoConfig.RenderOutline.DRAGGING
                ? 1f : 0f);
    }
    ...
    public boolean keyTyped(char character) {
        if (HoyoChar.enableSnapshot && character == 'B') {
    ...
    private HoyoConfig.Monitor refreshMonitorInfo() {
        HoyoConfig.Monitor[] monitors = HoyoConfig.Monitor.getMonitors();
        ...
        plane.world.clear();
        boolean flag = true;
        for (HoyoConfig.Monitor i : monitors) {
            if (!flag) break;
    ...
    private void promiseToolwindowStyle(int maxRetries) {
        if (config.window_style_toolwindow && !isToolwindowStyle) {
            // Make sure HoyoPets has been set as foreground window once
