...
import static cn.harryh.hoyopets.Const.charsetDefault;
import static cn.harryh.hoyopets.Const.hexColorRegex;


public class HoyoConfig implements Serializable {
    private static final URL configDefault = Objects.requireNonNull(HoyoConfig.class.getResource(cn.harryh.hoyopets.Const.configInternal));
        ...
        if (!configCustom.exists()) {
            ...
            try {
                return Objects.requireNonNull(
                        JSONObject.parseObject(FileUtil.readString(configCustom, charsetDefault), HoyoConfig.class),
     ...
     * @return An ArkConfig object. {@code null} if failed.
     */
    public static HoyoConfig getDefaultConfig() {
        try (InputStream inputStream = configDefault.openStream()) {
            return Objects.requireNonNull(
                    JSONObject.parseObject(new String(inputStream.readAllBytes(), charsetDefault), HoyoConfig.class),
                    "JSON parsing returns null."
            );
        } catch (IOException e) {
