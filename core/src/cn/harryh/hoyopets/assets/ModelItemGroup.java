...
import cn.harryh.hoyopets.assets.ModelItem.PropertyExtractor;
...
public class ModelItemGroup implements Collection<ModelItem> {
    ...
    public ModelItem searchByRelPath(String relPath) {
        for (ModelItem model : this) {
            if (Objects.equals(model.key, relPath)) {
                return model;
            }
        }
        return null;
     ...
     * @param mode The {@link FilterMode}.
     * @return A Model Item Group.
     * @param <T> The type of the property value.
     */
     ...
     */
    public void sort() {
        modelItemList.sort(Comparator.comparing(model -> model.key, Comparator.naturalOrder()));
    }
